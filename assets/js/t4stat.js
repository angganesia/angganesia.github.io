...
const OPTIONS = [
    ...
    { "name": "MaxHP %", "mat": "Metal", "pot": 10, "cost": 50, "cat": "Enhance HP/MP", "type": "u", bonus: 1, bonusratio: 1 / 3},
    ...
    { "name": "Physical Pierce %", "mat": "Beast", "pot": 20, "cost": 100, "cat": "Enhance Attack", "type": "w", bonus: 1, bonusratio: 2 / 5 },
    ...
    { "name": "Earth resistance %", "mat": "Mana", "pot": 5, "cost": 25, "cat": "Enhance Elements", "type": "a", bonus: 1, bonusratio: 2 / 3 },
    { "name": "Light resistance %", "mat": "Mana", "pot": 5, "cost": 25, "cat": "Enhance Elements", "type": "a", bonus: 1, bonusratio: 2 / 3 },
    { "name": "Dark resistance %", "mat": "Mana", "pot": 5, "cost": 25, "cat": "Enhance Elements", "type": "a", bonus: 1, bonusratio: 2 / 3 },
    ...
    { "name": "Aggro %", "mat": "Mana", "pot": 6, "cost": "33.49", "cat": "Special Enhancement", "type": "u", max: 15, max_only: true, bonus: 1, bonusratio: 2 / 5},
...
class Slot {
    ...
    buildDisplay() {
        ...
        <div class="row mt-2">
        <div class="col-9">
        <select class="form-select" id="slot${this.slot_num}" onchange="App.getCurrent().slots[${this.slot_num}].onUpdate()"><option value=0>MEMILIH STAT</option>`;
        let last_cat = '';
        let cat_id = 0;
        ...
        buffer += `
          <div class="col-3">
          <input autocomplete="off" type="number" maxlength=4 size=4 disabled id="input${this.slot_num}" value=0 onkeydown="App.getCurrent().slots[${this.slot_num}].onKeyPress(event)" oninput="App.getCurrent().slots[${this.slot_num}].onUpdate()" class="form-control"></input>
...
class Stat {
    ...
    updateMaterialCosts() {
        ...
        for (let mat in this.mats) {
            buffer += `
          <div class="row mt-1">
          <div class="col-6">${mat}</div>
          <div class="col-6">${this.mats[mat]}</div>
          </div>`
        }
         buffer += `
          <div class="row mt-1">
            <div class="col-6">Highest / Step</div>
            <div class="col-6">${this.max_mats}</div></div>`
    ...
    updateFormulaDisplay() {
        ...
        if (typeof this.finished === 'number') {
            display += `<p>Success Rate: ${this.getSuccessRate()}% </p>`;
            if (this.tec !== 255) display += ` <p class="text-danger">(${this.tec} TEC)</p>`
            display += `<p class="text-primary">Mats: ${Object.keys(this.mats).filter(mat => this.mats[mat]).map(mat => `${this.mats[mat]} ${mat}`).join(' / ')} (Max: ${this.max_mats})</p>`;
            ...
            if (message) display += ` <p class="text-success">(${message})</p>`
        }
        document.getElementById('formula_display').innerHTML = `<p class="mt-3">Steps</p>${this.type === 'w' ? 'Weapon' : 'Armor'} - Potential: ${this.starting_pot}<br />${display}`;
    ...
    loadDisplay() {
        ...
        const confirm = `<button class="btn btn-light" onclick="App.getCurrent().confirm()" id='confirm_button' disabled>Confirm</button>`;
        const repeat = `<button class="btn btn-light" id="repeat_button" onclick="App.getCurrent().repeat()" disabled>Repeat</button>`;
        const undo = `<button class="btn btn-light" onclick="App.getCurrent().undo()" disabled id="undo_button">Undo</button>`;
        const redo = `<button class="btn btn-light" onclick="App.getCurrent().redo()" disabled id="redo_button">Redo</button>`;

        const display = `
        <div class="text-center mb-3" id='potential_display'>Potential: ${potential}</div>
        <div class="row">
        <div>${buffer}</div>
        </div>
        <div class="text-center mt-3" id="success_rate_display">Success Rate: ${this.getSuccessRate()}%</div>
        <div class="text-center">${confirm} ${repeat} ${undo} ${redo}</div>`;
...
class MainApp {
    ...
    buildNavigationBar() {
        ...
        for (const workspace_id in this.stats) {
            ...
            buffer.push(`<div class="mt-2 col-10 d-grid gap-2"><button class="btn btn-light" onclick="App.setCurrent('${workspace_id}')">${workspace_id}</button></div><div class="mt-2 col-2 d-grid gap-2"><button class="btn btn-danger ms-1" onclick="App.despawn('${workspace_id}')" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3-fill" viewBox="0 0 16 16">
